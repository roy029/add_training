...
def from_tsv(f_path):
  ...
  df = df.sample(frac=1, random_state=0).reset_index(drop=True) #データの全てをシャッフル

  df_train, df_test = train_test_split(df, test_size=0.3, random_state=42)
  df_valid, df_test = train_test_split(df_test, test_size=0.5, random_state=42)

  #ここでdfをtokenizerに入れてtgt-srcのペアのdfに変更、trainに3分割してtokenizerで処理するときに
  # (これをするとtokenizerを変更する意味がないのでx)もしくはINPUT_tsvをdfにする前に、INPUT-->mask_INPUTにする
...
class TsvDataset(Dataset): #FIXME
