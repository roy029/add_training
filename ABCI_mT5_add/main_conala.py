...
class MaskedDataset(Dataset):
    def __init__(self, tokenizer, data_dir, type_path, input_max_len=512, target_max_len=512):
        ...
        self.lines = []
        self.ratio = 0.25  # マスクをかける比率を設定する
        self._build()
    ...
    def __getitem__(self, index):
        line = self.lines[index] # 読む
        inputs = self.tokenizer.batch_encode_plus(
            [line], max_length=self.input_max_len, truncation=True, return_tensors="pt")
        #print(inputs)
        input_ids = inputs["input_ids"].squeeze().tolist()
        input_ids = input_ids[:-1] # </s> を除いたinputs のlist
        n_tokens = len(input_ids)   # 字句数
        n = max(int((n_tokens / 2) * self.ratio), 1)
        input_masked = sorted(random.sample(list(range(0, n_tokens)), n))
        output_masked = list(set(list(range(0, n_tokens))) - set(input_masked))
        source, source_attn = masking_pad(input_ids[:], input_masked, self.input_max_len)
        target, target_attn = masking_pad(input_ids[:], output_masked, self.input_max_len)
        #print('source', source, self.tokenizer.decode(source))
        #print('target', target, self.tokenizer.decode(target))
        f = torch.tensor
...
if __name__ == '__main__':
    ...
    OUTPUT_tsv = f'result_mT5_greedy_{INPUT_tsv_name}.tsv'

    if args.model:
        shutil.make_archive(f'trained_model/model_mT5_greedy_{INPUT_tsv_name}', 'zip', root_dir='model')
    elif args.result:
    ...
    with open('log/trainlog_mT5.txt', mode= 'a') as f:
        ...
        f.write(f'Pretrained Model : trained_model/model_mT5_greedy_{INPUT_tsv_name}.zip\n')
